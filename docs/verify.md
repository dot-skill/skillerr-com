...
The result includes two separate lists, not one list with a flag: **verified** (checked by math — digests, signatures, trust-store pins, anchor verification) and **self-reported** (asserted by the environment or signer at mint time — host, model, runtime — never independently checkable). Every field is in exactly one list, never both, so nothing on this page can present a self-reported claim next to a "verified" checkmark. Built on `assessClaims()` in `@skillerr/core` — the same function `skill inspect --trust --claims` / `skill verify-trust --claims` use.
